<script lang="ts">
	 ...
	 * paired card. Skipped terms are still listed on Reference with the book's
	 * wording, waiting to be captured.
	 */
	...
	let {
		...
		href,
		children
	}: { name: string; href?: string; children?: import('svelte').Snippet } = $props();
	...
	function save() {
		...
		notes.add({
			...
			origin: 'own',
			source: { lo: term.lo, label: 'Taught here', href: resolvedHref }
		});
		open = false;
		btn?.focus();
	}
	...
	>{#if open}
		...
		>
			<div class="px-4 pt-3 pb-1">
				<div class="kicker">Key term · {term.lo}{captured ? ' · in your notes' : ''}</div>
